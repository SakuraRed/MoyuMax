...
test("M33-MEM-001 全局内存默认自动分配并展示当前取值", async ({ page }) => {
  ...
  const group = page.getByRole("group", { name: "启动内存" });
  await expect(group.getByRole("button", { name: "自动分配（推荐）" })).toHaveClass(/on/);
...
test("M33-MEM-002 全局自定义保存、回读与恢复自动", async ({ page }) => {
  ...
  // 切到自定义,用当前自动分配值预填
  await page.getByRole("group", { name: "启动内存" }).getByRole("button", { name: "自定义" }).click();
  ...
  await expect(page.getByText("全局启动内存已保存", { exact: true })).toBeVisible();
  ...
  await openMemorySettings(page);
  await expect(page.getByRole("group", { name: "启动内存" }).getByRole("button", { name: "自定义" })).toHaveClass(/on/);
  ...
  await page.getByRole("button", { name: "保存", exact: true }).click();
  await expect(page.getByText("内存设置必须满足", { exact: false })).toBeVisible();
  ...
  // 切回自动分配,清除全局自定义
  await page.getByRole("group", { name: "启动内存" }).getByRole("button", { name: "自动分配（推荐）" }).click();
  await expect(page.getByText("已切换为自动分配", { exact: true })).toBeVisible();
  await expect(page.getByRole("textbox", { name: "全局最小内存 MiB" })).toHaveCount(0);
